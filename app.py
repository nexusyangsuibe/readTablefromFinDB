...
if __name__=="__main__":
    ...
    concat_df_cfg={
        ...
        "target_folder":r"rawdata", # 待处理数据所在的文件夹，如果是zip文件也需要放在一个文件夹中
        ...
        "output_filename":"test.pkl", # 表格合并程序输出文件名，为防止文本中逗号与逗号分隔符混淆不支持csv，支持xlsx，数据量较大时会以1000000行为分割输出多个excel文件，支持pickle(.pkl)格式
    ...
    rtn_catdf=concatDF(concat_df_cfg["runtime_code"],concat_df_cfg["data_source"],concat_df_cfg["target_folder"],concat_df_cfg["usecols"],concat_df_cfg["ts_index_column_name"],concat_df_cfg["filter_conditions"],concat_df_cfg["csv_delimiter"],concat_df_cfg["convert_str_columns"],concat_df_cfg["output_filename"],concat_df_cfg["skiprows"],concat_df_cfg["zip_starts_with"],concat_df_cfg["clear_respawnpoint_before_run"],concat_df_cfg["clear_respawnpoint_upon_conplete"])
    print("\n程序运行完成，用时{:.4f}秒".format(time.perf_counter()-t0))